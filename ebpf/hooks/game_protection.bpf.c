...
static __always_inline caller_ctx get_caller_ctx();
...
/* move_mount
 *
 * Prevents mount(2) operations from targeting or relocating TARGET_CGROUP.
 *
 * Blocks:
...
static __always_inline caller_ctx get_caller_ctx() {
  ...
  caller.pid  = (__u32) (pid_tgid >> 32);
  caller.cgid = bpf_get_current_cgroup_id();
  return caller;
}
